...
export const productService = {
  ...
  async listProducts({
    ...
    const hasSubWhere = Object.keys(subcategoryWhere).length > 0;
    const subInclude = {
      model: Subcategory,
      as: 'subcategory',
      where: hasSubWhere ? subcategoryWhere : undefined,
      required: hasSubWhere,
      include: [
        {
          model: Category,
          as: 'category',
  ...
  async updateProduct(
    ...
    if (badge !== undefined) updates.badge = badge ? String(badge).trim() : null;
